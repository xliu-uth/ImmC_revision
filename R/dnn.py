...
seed = 100
np.random.seed(seed)
input_file = pd.read_csv("./tensorflow/input/deeplearn.train.balance.input08112020.txt", sep = ",")
data =input_file.values


X_train = data[:, 0:171].astype(float)
y_train = np.array([[int(x) for x in str.split(y, sep = ",")] for y in data[:, 173]])
